...
;(function () {
  ...
  const CONFIG = {
    ...
    EPIC_COLORS: {
      'ghx-label-0': {
        color: 'var(--ds-background-accent-gray-bolder, #6b778c)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-1': {
        color: 'var(--ds-background-accent-orange-bolder, #8d542e)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-2': {
        color: 'var(--ds-background-accent-orange-subtle, #ff8b00)',
        textColor: 'var(--ds-text-accent-orange-bolder, #172b4d)',
      },
      'ghx-label-3': {
        color: 'var(--ds-background-accent-yellow-subtle, #ffc400)',
        textColor: 'var(--ds-text-accent-yellow-bolder, #172b4d)',
      },
      'ghx-label-4': {
        color: 'var(--ds-background-accent-blue-bolder, #0747a6)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-5': {
        color: 'var(--ds-text-accent-gray-bolder, #253858)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-6': {
        color: 'var(--ds-background-accent-green-subtler, #57d9a3)',
        textColor: 'var(--ds-text-accent-green-bolder, #172b4d)',
      },
      'ghx-label-7': {
        color: 'var(--ds-background-accent-magenta-bolder, #b93d9e)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-8': {
        color: 'var(--ds-background-accent-purple-bolder, #5243aa)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-9': {
        color: 'var(--ds-background-accent-red-subtler, #ff8f73)',
        textColor: 'var(--ds-text-accent-red-bolder, #172b4d)',
      },
      'ghx-label-10': {
        color: 'var(--ds-background-accent-blue-subtler, #0065ff)',
        textColor: 'var(--ds-text-accent-blue-bolder, #fff)',
      },
      'ghx-label-11': {
        color: 'var(--ds-background-accent-teal-bolder, #008299)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-12': {
        color: 'var(--ds-background-accent-gray-bolder, #5e6c84)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-13': {
        color: 'var(--ds-background-accent-green-bolder, #00875a)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
      'ghx-label-14': {
        color: 'var(--ds-background-accent-red-bolder, #de350b)',
        textColor: 'var(--ds-text-inverse, #fff)',
      },
    },
  ...
  const Utils = {
    ...
    getColorInfo: (label) => {
      ...
      if (!label || label.trim() === '') {
        return {
          color: 'var(--ds-background-neutral, #ccc)',
          textColor: 'var(--ds-text, #000)',
        }
      }
      return (
        CONFIG.EPIC_COLORS[label] || {
          color: 'var(--ds-background-neutral, #ccc)',
          textColor: 'var(--ds-text, #000)',
        }
      )
  ...
  const Selectors = {
    ...
    isEpicIssue: (row) => {
      ...
      const epicIcon = row.querySelector(
        "img[alt*='Epic'], img[title*='Epic'], .aui-icon-epic, [data-issue-type*='epic']"
      )
      if (epicIcon) return true

      // 2. Check if issue type column contains "Epic"
      const issueTypeCell = row.querySelector(
        "td.issuetype, td[data-field-id='issuetype']"
      )
      if (
        issueTypeCell &&
        issueTypeCell.textContent.toLowerCase().includes('epic')
      )
        return true

      // 3. Check for epic-specific CSS classes
      if (row.classList.contains('epic') || row.querySelector('.epic'))
        return true
      ...
      const epicColorCell = row.querySelector(
        `td.${CONFIG.FIELD_ID}, td[data-field-id='${CONFIG.FIELD_ID}']`
      )
      if (
        epicColorCell &&
        epicColorCell.textContent.trim() &&
  ...
  class DropdownCreator {
    static create(currentLabel) {
      ...
      dropdown.style.setProperty(
        'color',
        Utils.getTextColor(colorInfo.color, currentLabel),
        'important'
      )
      ...
      if (!currentLabel || currentLabel === '') {
        ...
        undefinedOption.style.setProperty(
          'background-color',
          'var(--ds-background-neutral, #ccc)',
          'important'
        )
        undefinedOption.style.setProperty(
          'color',
      ...
      Object.entries(CONFIG.EPIC_COLORS).forEach(([labelKey, colorData]) => {
        ...
        option.style.setProperty(
          'color',
          Utils.getTextColor(colorData.color, labelKey),
          'important'
        )
    ...
    static updateVisualState(dropdown, label) {
      ...
      dropdown.style.setProperty(
        'color',
        Utils.getTextColor(colorInfo.color, label),
        'important'
      )
    ...
    static showPreview(dropdown, selectedLabel) {
      ...
      dropdown.style.setProperty(
        'color',
        Utils.getTextColor(colorInfo.color, selectedLabel),
        'important'
      )
