...
;(function () {
  ...
  async function fetchBoardConfiguration(boardId) {
    console.log(
      `${PREFIX} === FETCHING BOARD CONFIGURATION (delegating to fetchCardColors) ===`
    )
  ...
  async function fetchCardColors(boardId) {
    ...
    try {
      ...
      const response = await fetch(
        `/rest/greenhopper/1.0/cardcolors/${boardId}/strategy/custom?preloadValues=true&_=${timestamp}`,
        {
          method: 'GET',
          headers: {
      ...
      if (response.ok) {
        ...
        if (cardColors.cardColors && Array.isArray(cardColors.cardColors)) {
          console.log(
            `${PREFIX} Found ${cardColors.cardColors.length} card color rules:`
          )
        ...
        console.log(
          `${PREFIX} Failed to fetch card colors: ${response.status} ${response.statusText}`
        )
        return null
      }
    } catch (error) {
  ...
  async function getProjectIdFromKey(projectKey) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `${PREFIX} Found project ID ${project.id} for key ${projectKey}`
        )
        return project.id
      } else {
        console.log(
          `${PREFIX} Failed to get project ID for ${projectKey}: ${response.status}`
        )
        return null
      }
    } catch (error) {
  ...
  window.debugBoardImages = function () {
    ...
    console.log(
      `${PREFIX} [DEBUG] Found ${allImages.length} total images in .ghx-type containers`
    )
  }
  ...
  function saveConfigForBoard(boardConfig) {
    ...
    const existingIndex = allConfigs.findIndex(
      (config) => config.boardId === boardConfig.boardId
    )
    ...
    console.log(
      `${PREFIX} Saved config for board ${boardConfig.boardId}:`,
      boardConfig
    )
  }
  ...
  async function showSettingsDialog() {
    ...
    const currentBoardId = new URLSearchParams(window.location.search).get(
      'rapidView'
    )
    if (!currentBoardId) {
  ...
  async function showFallbackDialog() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          colorResponse &&
          colorResponse.cardColors &&
          Array.isArray(colorResponse.cardColors)
        ) {
          cardColorRules = colorResponse.cardColors
          console.log(
            `${PREFIX} Found ${cardColorRules.length} card color rules`
          )
        }
      } else {
    ...
    if (cardColorRules.length > 0) {
      console.log(
        `${PREFIX} Building HTML for ${cardColorRules.length} rules:`,
        cardColorRules
      )
      cardColorRulesHtml = cardColorRules
        .map(
          (rule) => `
      ...
      cardColorRulesHtml =
        '<div style="color: var(--ds-text-subtle, #6b778c); font-style: italic;">No card color rules found. Please check Board ID.</div>'
    }
    ...
    document.getElementById('save-custom').onclick = async () => {
      ...
      const selectedRuleRadio = document.querySelector(
        'input[name="colorRule"]:checked'
      )
      const selectedRuleId = selectedRuleRadio ? selectedRuleRadio.value : null
      const selectedRule = cardColorRules.find(
        (rule) => rule.id == selectedRuleId
      )

      // Get image configuration
      const projectKey = document
        .getElementById('projectKey-custom')
        .value.trim()
        .toUpperCase()
      const avatarId = document.getElementById('avatarId-custom').value.trim()
      const customImageUrl = document
        .getElementById('imageUrl-custom')
        .value.trim()
      ...
      if (customImageUrl) {
        ...
        try {
          ...
          if (projectId) {
            ...
            alert(
              `Could not find project ID for key "${projectKey}". Please check the project key.`
            )
            return
          }
        } catch (error) {
          ...
          alert(
            `Error resolving project key "${projectKey}". Using project key as fallback.`
          )
      ...
      try {
        ...
        if (
          ...
          typeof window.GH.RapidBoard.reload === 'function'
        ) {
        ...
        // Method 3: Try to find and click a refresh button
          ...
          const refreshButton = document.querySelector(
            '[data-testid="board.header.refresh-button"], .ghx-refresh, #ghx-refresh'
          )
          if (refreshButton) {
            refreshButton.click()
        ...
        console.log(
          `${PREFIX} Could not auto-refresh board, fallback to manual reload:`,
          error
        )
        if (window.AJS && window.AJS.flag) {
          const flag = window.AJS.flag({
  ...
  async function boardUtils() {
    try {
      ...
      const config = getConfigForBoard(currentBoardId)
      console.log(
        `${PREFIX} Config loaded for board ${currentBoardId}:`,
        config
      )
      ...
      if (!config.enabled) {
        console.log(
          `${PREFIX} Board Utils is disabled for board ${currentBoardId}, skipping`
        )
        return
      }
      ...
      if (selectedRuleColor) {
        ...
        if (colorResponse && Array.isArray(colorResponse)) {
          ...
          console.log(
            `${PREFIX} No rule selected and failed to fetch colors, using fallback`
          )
          configuredColors = []
        }
      }
      ...
      function observeGrabbers() {
        ...
        if (!targetNode) {
          console.log(
            `${PREFIX} Target node not found: ${SELECTORS.grabberPool}`
          )
          return
        }
      ...
      function addImageToCardFooter(cardFooter, context = '') {
        if (!cardFooter.querySelector('.custom-board-img')) {
          console.log(
            `${PREFIX}${context} Adding image to card footer`,
            cardFooter
          )
          ...
          if (daysElement) {
            ...
            console.log(
              `${PREFIX}${context} No specific elements found, appended to end`
            )
          }
          console.log(
            `${PREFIX}${context} Card footer after adding image:`,
          ...
          console.log(
            `${PREFIX}${context} Image visible?`,
            existingImg?.offsetWidth > 0 && existingImg?.offsetHeight > 0
          )
        }
      }
      ...
      function processCardGrabber(cardGrabber, configuredColors, context = '') {
        const elementColor =
          window.getComputedStyle(cardGrabber).backgroundColor
        const matchesColor = matchesAnyColor(elementColor, configuredColors)

        console.log(
          `${PREFIX}${context} Checking color: ${elementColor} matches configured colors? ${matchesColor}`
        )

        if (matchesColor) {
          console.log(
            `${PREFIX}${context} Found matching color, looking for card footer`
          )
